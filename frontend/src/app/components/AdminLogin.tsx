...
import { Lock, User, Sparkles, ChevronRight, RefreshCw } from "lucide-react";
        ...
        <Card className="w-full border-white/10 bg-[linear-gradient(135deg,rgba(18,18,30,0.82),rgba(10,10,18,0.92))] backdrop-blur-2xl shadow-[0_20px_50px_-15px_rgba(139,92,246,0.25)] login-card-float">
          <CardHeader className="space-y-4 p-8 text-center">
            ...
            <div>
              ...
              <CardDescription className="mt-2 text-sm text-white/60 leading-relaxed">
                Acceso seguro con sesión firmada por el servidor
              </CardDescription>
            </div>
          </CardHeader>
          <CardContent className="space-y-6 p-8 pt-0">
            ...
            <div className="flex flex-col sm:flex-row items-center justify-between gap-3 border-t border-white/5 pt-4 text-xs font-semibold text-muted-foreground">
              <button onClick={() => navigate("/barber/login")} className="hover:text-primary hover:underline transition-colors cursor-pointer bg-transparent border-none">
                Ir al panel de barbero
              </button>
              <button onClick={() => navigate("/")} className="hover:text-white transition-colors cursor-pointer bg-transparent border-none">
                ← Volver a reservas
              </button>
            </div>
          </CardContent>
